...
void Chat::sendText()
{
    ...
    QString timestr = time.toString("yyyy-MM-dd hh:mm:ss ddd") ;    // 设置当前系统时间格式并转换成字符串形式
    QString inputstr = inputtext->toPlainText() ;   // 获取输入框的聊天信息
    // 封装聊天信息的样式
    QString str = "<font style=\"font-family:"+font.family()+";font-size:"
            +QString::number(font.pointSize())+"\">"+inputstr+"</font>" ;
    // 完整的聊天信息
    alltext->append(str);
}
